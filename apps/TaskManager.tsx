...
export default function TaskManager() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0A0A0A] text-zinc-300 font-mono text-sm overflow-hidden">
      {/* Header Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-4 p-3 sm:p-4 bg-zinc-900 border-b border-white/5 shrink-0">
        <div className="bg-black/40 border border-white/5 rounded-xl p-3 sm:p-4 flex gap-3 sm:gap-4 items-center relative overflow-hidden group">
           <div className="absolute right-0 top-0 h-full w-32 bg-gradient-to-l from-emerald-500/10 to-transparent pointer-events-none" />
           <Cpu className="text-accent group-hover:scale-110 transition-transform shrink-0" size={28} />
           <div>
              <div className="text-[9px] sm:text-[10px] text-zinc-500 uppercase tracking-widest font-bold">CPU Load</div>
              <div className="text-xl sm:text-2xl font-light text-white">{cpuUsage}%</div>
           </div>
        </div>
        <div className="bg-black/40 border border-white/5 rounded-xl p-3 sm:p-4 flex gap-3 sm:gap-4 items-center relative overflow-hidden group">
           <div className="absolute right-0 top-0 h-full w-32 bg-gradient-to-l from-cyan-500/10 to-transparent pointer-events-none" />
           <HardDrive className="text-accent group-hover:scale-110 transition-transform shrink-0" size={28} />
           <div>
              <div className="text-[9px] sm:text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Memory Allocated</div>
              <div className="text-xl sm:text-2xl font-light text-white">{memUsageGB} GB</div>
           </div>
        </div>
        <div className="bg-black/40 border border-white/5 rounded-xl p-3 sm:p-4 flex gap-3 sm:gap-4 items-center relative overflow-hidden group">
           <div className="absolute right-0 top-0 h-full w-32 bg-gradient-to-l from-purple-500/10 to-transparent pointer-events-none" />
           <Activity className="text-purple-400 group-hover:scale-110 transition-transform shrink-0" size={28} />
           <div>
              <div className="text-[9px] sm:text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Active Threads</div>
              <div className="text-xl sm:text-2xl font-light text-white">{processes.length * 4 + 12}</div>
           </div>
        </div>
      </div>
